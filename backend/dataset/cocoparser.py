...
class CocoImage:
    def __init__(self, image_dict: dict) -> None:
        self.filename = image_dict.get("file_name")
        self.height = image_dict.get("height", 0)
        self.width = image_dict.get("width", 0)
...
class CocoAnnotation:
    def __init__(self, anno_dict: dict, category: CocoCategory, image: CocoImage) -> None:
        ...
        self.width = bbox[2]
        self.height = bbox[3]
        self.area = anno_dict.get("area", 0)
...
class CocoDataset:
    ...
    def _parse(self) -> None:
        ...
        category_map = {}
        for category in annotations.get("categories"):  # type: ignore
            id = category.get("id")
        ...
        image_map = {}
        for i, image in enumerate(annotations.get("images")):  # type: ignore
            id = image.get("id")
            coco_image = CocoImage(image_dict=image)
            image_map[id] = coco_image
            self.images.append(coco_image)

        for annotation in annotations.get("annotations"):  # type: ignore
            image_id = annotation.get("image_id")
            category_id = annotation.get("category_id")

            self.annotations.append(CocoAnnotation(
                anno_dict=annotation, category=category_map.get(category_id), image=image_map.get(image_id)  # type: ignore
            ))
